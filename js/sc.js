const SOFTCAPS = {
	...
	e32: {
		...
		info() { return "开始于 "+format(this.start)+"x, exponent brought to the fourth root" },
	},
	spaceBuilding3: {
	...
	hex: {
		...
		info() { return "开始于 "+format(this.start())+"x, 对数 but raised to the power of "+format(this.exp()) },
	},
	spell1: {
		title: "第一魔法（装载增幅器）",
		type: "expRoot",
	...
	ba12: {
		...
		info() { return "开始于 "+format(this.start.times(100))+"%, 对数 and brought to the "+format(this.exp.pow(-1))+"th root" },
	},
	ba32: {
		title: "平衡升级（实体重生）",
		type: "log",
	...
	hsBuilds: {
		...
		info() { return "开始于 Level "+format(this.start().plus(1))+", brought to the fifth root" },
	},
	rotEff: {
		...
		info() { return "开始于 "+format(this.start)+"x, exponent brought to the 1.7th root" },
	},
}
